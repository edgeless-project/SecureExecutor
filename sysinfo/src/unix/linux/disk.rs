...
impl crate::DisksInner {
    ...
    pub(crate) fn refresh_list(&mut self) {
        ...
        // FIXME: The above code in EDGELESS (which runs inside a container) returns zeros. 
        ...
        self.sysinfo_workaround_to_read_disks("/disks/refresh_list");
    }
    ...
    // FIXME: In EDGELESS sysinfo invokes (which runs inside a container) has in some occasions problems. 
    // The following code is a workaround, which updates information 
    // with the help of a separate program that runs outside of the enclave, in the untrusted world.
    // The following function actually, either invokes disk.refresh() or disk.refresh_list() and updates
    // self.disk contents based on the response that will retrieved from the untrusted part.
    pub(crate) fn sysinfo_workaround_to_read_disks(&mut self, url: &str){
        // First clear the disk if already exist
        self.disks.clear();

        // Ask the untrusted part and take back the response
        let _response = crate::requests::request(url);

        // Trim message part and keep only the disk info, then for each disk in the response
        let disks_part = _response.trim_start_matches("Disks {").trim_end_matches('}');
        for disk_str in disks_part.split("Disk").skip(1) {
            // Remove explanation text
            let mut disk_str = disk_str.replace(" mounted on ","");
            disk_str = disk_str.replace("FS: ","");
            disk_str = disk_str.replace("Type: ","");
            disk_str = disk_str.replace("removable: ","");
            
            // Separate fields
            disk_str = disk_str.replace(")",",");
            disk_str = disk_str.replace("]",",");
            disk_str = disk_str.replace(": ",",");
            // Convert last occurence of / (this separes available disk space with total) to a comma
            disk_str.rfind('/').map(|n| disk_str.replace_range(n..n+1, ","));

            // Remove other not important characters
            disk_str = disk_str.replace("\"","");
            disk_str = disk_str.replace("(","");
            disk_str = disk_str.replace("[","");
            disk_str = disk_str.replace(" B","");

            // Convert String to a vector to access each field and do the actual parsing
            let disk: Vec<String> = disk_str.split(',').map(|s| s.trim().to_string()).collect();
            let disk_device_name= &disk[0];
            let disk_file_system = &disk[1];
            let disk_type = match disk[2].as_str() {
                "SSD" => self::DiskKind::SSD,
                "HDD" => self::DiskKind::HDD,
                _ => self::DiskKind::Unknown(1),
            };
            let disk_is_removable =  match disk[3].as_str() {
                "yes" => true,
                "mo" => false,
                _ => false,
            };
            let disk_mount_point = &disk[4];
            let disk_available_space: u64 = disk[5].parse::<u64>().unwrap_or(0);
            let disk_total_space: u64 = disk[6].parse::<u64>().unwrap_or(0);

            // Create actual struct and push it into the disks 
            self.disks.push(
                crate::Disk{
                    inner: crate::DiskInner{
                        type_: disk_type, 
                        device_name: std::ffi::OsString::from(disk_device_name),
                        file_system: std::ffi::OsString::from(disk_file_system),
                        mount_point: std::path::PathBuf::from(disk_mount_point), 
