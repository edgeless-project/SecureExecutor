...
impl SystemInner {
    ...
    pub(crate) fn refresh_memory_specifics(&mut self, refresh_kind: MemoryRefreshKind) {
        ...
        read_table("/proc/meminfo", ':', |key, value_kib| {
            let field = match key {
                ...
                // }
                // FIXME: The above code in EDGELESS (which runs inside a container) due to shielding returns zeros. 
        ...
        self.mem_total      = crate::requests::request("/system/total_memory").parse().unwrap_or(0);
        self.mem_free       = crate::requests::request("/system/free_memory").parse().unwrap_or(0);
        self.mem_available  = crate::requests::request("/system/available_memory").parse().unwrap_or(0);
    }
    ...
    pub(crate) fn load_average() -> LoadAvg {
        ...
        // The following code is a workaround, which updates information 
        // with the help of a separate program that runs outside of the enclave, in the untrusted world.
        let _response = crate::requests::request("/system/load_average");
        let parts: Vec<&str> = _response.split(',').collect();
        
        // In case the response does not contain 3 u64 numbers "X,Y,Z" then return default values
        if parts.len() < 3 {
            return LoadAvg{
                one: 0.0,
                five: 0.0,
        ...
        // Parse each part into f64 and handle the Result with unwrap (or handle errors)
