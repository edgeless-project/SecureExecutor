...
pub fn update_processes_from_untrusted_part_edgeless_workaround(processes: &mut HashMap<Pid, Process>){
    // =====================================================================================
    ...
    // Get processes from the untrusted part
    let mut _response = crate::requests::request("/system/refresh_all/processes");

    // Trim wrapper part and keep only the message info
    let processes_part = _response.trim_start_matches("Processes: {");

    // Spit message into its fields and parse it
    let delimiter = "^";
    for process_str in processes_part.split(": Process {").skip(1) {

        // =========================================================================================================================================
        // Separete each field
        let mut process_str = process_str.replace("pid: ",delimiter);
        process_str = process_str.replace(", parent: ", delimiter);
        process_str = process_str.replace(", name: ", delimiter);
        process_str = process_str.replace(", environ: ", delimiter);
        process_str = process_str.replace(", command: ", delimiter);
        process_str = process_str.replace(", executable path: ", delimiter);
        process_str = process_str.replace(", current working directory: ", delimiter);
        process_str = process_str.replace(", memory usage: ", delimiter);
        process_str = process_str.replace(", virtual memory usage: ", delimiter);
        process_str = process_str.replace(", CPU usage: ", delimiter);
        process_str = process_str.replace(", status: ", delimiter);
        process_str = process_str.replace(", root: ", delimiter);
        process_str = process_str.replace(", disk_usage: DiskUsage {", "");
        process_str = process_str.replace(" total_written_bytes: ", delimiter);
        process_str = process_str.replace(", written_bytes: ", delimiter);
        process_str = process_str.replace(", total_read_bytes: ", delimiter);
        process_str = process_str.replace(", read_bytes: ", delimiter);
        process_str = process_str.replace(", user_id: ", delimiter);
        process_str = process_str.replace(", effective_user_id: ", delimiter);
        process_str = process_str.replace("}", "");

        // =========================================================================================================================================
        // Having a unique separator, parse parameters
        let process_fields: Vec<String> = process_str.split(delimiter).skip(1).map(|s| s.trim().to_string()).collect();         
        
        // [ok] PID   
        let _pid = crate::Pid::from(
            process_fields[0]
                .replace("Pid(", "")
                .replace(")", "")
                .parse::<usize>()
        ...
        // [ok] Parent ID 
        let _parent = match process_fields[1].as_str() {
            "None" => None,
            _ => {
                Some(
                    crate::Pid::from(
        ...
        // Name
        let _name = &process_fields[2];
        
        // Environment
        let _environment = &process_fields[3];
        
        // Command
        let _command = &process_fields[4];
        
        // Executable path
        let _executable_path = Path::new(&process_fields[5]);
        
        // CWD
        let _current_working_directory = &process_fields[6];
        
        // [ok] Memory usage
        let _memory_usage = process_fields[7].parse::<u64>().unwrap_or(0);
        
        // [ok] Virtual memory usage
        let _virtual_memory_usage = process_fields[8].parse::<u64>().unwrap_or(0);
        
        // [ok] CPU usage
        let _cpu_usage = process_fields[9].parse::<f32>().unwrap_or(0.0);
        
        // [ok] Status
        let _status =  match process_fields[10].as_str() {
            "Idle" => self::ProcessStatus::Idle,
            "Run" => self::ProcessStatus::Run,
            "Sleep" => self::ProcessStatus::Sleep,
            "Stop" => self::ProcessStatus::Stop,
            "Zombie" => self::ProcessStatus::Zombie,
            "Tracing" => self::ProcessStatus::Tracing,
            "Wakekill" => self::ProcessStatus::Wakekill,
            "Waking" => self::ProcessStatus::Waking,
            "Parked" => self::ProcessStatus::Parked,
            "LockBlocked" => self::ProcessStatus::LockBlocked,
            "UninterruptibleDiskSleep" => self::ProcessStatus::UninterruptibleDiskSleep,
            _ => self::ProcessStatus::Unknown(0),
        };

        // root
        let _root = &process_fields[11];

        // [ok] Total written bytes
        let _total_written_bytes = process_fields[12].parse::<u64>().unwrap_or(0);
        
        // [ok] written bytes
        let _written_bytes = process_fields[13].parse::<u64>().unwrap_or(0);
        
        // [ok] total read bytes
        let _total_read_bytes = process_fields[14].parse::<u64>().unwrap_or(0);
        
        // [ok] read bytes
        let _read_bytes = process_fields[15].parse::<u64>().unwrap_or(0);

        // user id
        let _user_id = match process_fields[16].as_str() {
            "None" => None,
            _ => {
                let id = process_fields[16]
                            .replace("Some", "")
                            .replace("Uid", "")
                            .replace("(", "")
                            .replace(")", "");
                let id_str = id.as_str(); 
                let uid: Result<Uid, std::num::ParseIntError> = crate::Uid::from_str(id_str);
                match uid {
                    Ok(uid)=> Some(uid),
                    _ => None
                }
        ...
        // Effective user id
        let _effective_user_id = match process_fields[17].as_str().split(",").nth(0).unwrap_or("").trim() {
            "None" => None,
            _ => {
                let id = process_fields[17].as_str().split(",").nth(0).unwrap_or("").trim()
                            .replace("Some", "")
                            .replace("Uid", "")
                            .replace("(", "")
                            .replace(")", "");
        ...
        // Debug messages
        // println!("1) pid: {:?}", _pid);
        // println!("2) parent: {:?}", _parent);
        // println!("3) name: {:?}", _name);
        // println!("4) environment: {:?}", _environment);
        // println!("5) command: {:?}", _command);
        // println!("6) executable_path: {:?}", _executable_path);
        // println!("7) current_working_directory: {:?}", _current_working_directory);
        // println!("8) memory_usage: {:?}", _memory_usage);
        // println!("9) virtual_memory_usage: {:?}", _virtual_memory_usage);
        // println!("10) cpu_usage: {:?}", _cpu_usage);
        // println!("11) status: {:?}", _status);
        // println!("12) root: {:?}", _root);
        // println!("13) total_written_bytes: {:?}", _total_written_bytes);
        // println!("14) written_bytes: {:?}", _written_bytes);
        // println!("15) total_read_bytes: {:?}", _total_read_bytes);
        // println!("16) read_bytes: {:?}", _read_bytes);
        // println!("17) user_id: {:?}", _user_id);
        // println!("18) effective_user_id: {:?}", _effective_user_id);
        // println!("------------------------------\n");

        
        // =========================================================================================================================================
        // Create Process struct
        let mut p = ProcessInner::new(_pid, _executable_path.to_owned());
        p.name =  std::ffi::OsString::from(_name);
        p.memory = _memory_usage;
        p.status = _status;
        p.old_read_bytes = _total_read_bytes;
        p.old_written_bytes = _total_written_bytes;
        p.read_bytes = _read_bytes;
        p.written_bytes = _written_bytes;
        p.cpu_usage = _cpu_usage;
        
        // =========================================================================================================================================
        // Append new process into process list
        let process = crate::Process{ inner: p };
        let _result = match processes.insert(_pid, process){
            Some(_val) => Ok(0),
            None => Err("Eroor"),
        };
    }
